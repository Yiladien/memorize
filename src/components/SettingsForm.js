...
const SettingsForm = ({ gameSettings, updateGameSettings }) => {
  ...
  useEffect(() => {
    setFormValues({ ...gameSettings });
  }, [gameSettings]);

  //  --- Form Handlers
  const handleDropdown = (e1) => {
    // setFormValues({ ...formValues, shape: e1 });
    updateGameSettings({ ...formValues, shape: e1 });
  };
  ...
  const handleSwitch = (e1, e2, e3) => {
    ...
    updateGameSettings({
      ...formValues,
      [e1.target.name]: !gameSettings[e1.target.name],
    });
  };
  ...
  const handleChange = (e1, minValue, maxValue) => {
    ...
    if (e1.target.value < minValue || e1.target.value > maxValue) {
      ...
      if (e1.target.name === "itemCount") {
        ...
        let colorList = { ...formValues.gameColors };
  ...
  return (
    <>
      ...
      <Form.Label className="me-2 w-100">How many to Memorize:</Form.Label>
      <InputGroup
        variant="secondary"
        size="sm"
      ...
      >
        ...
        <Form.Control
          ...
          onChange={(event) =>
            handleChange(event, formValues.minItems, formValues.maxItems)
          }
          name="itemCount"
          isInvalid={
      ...
      <Form.Group className="mb-2 border-bottom">
        ...
        <Form.Check
          ...
          type="switch"
          onChange={handleSwitch}
          checked={formValues.answerTimer}
        />
        {formValues.answerTimer ? (
          <InputGroup
            variant="secondary"
            size="sm"
